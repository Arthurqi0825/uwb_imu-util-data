...
// =============================================================================
// TdoaWlsSolver — Gauss-Newton WLS (validated, unchanged)
// =============================================================================
class TdoaWlsSolver {
 public:
  ...
  bool solve(const std::vector<TdoaMeas>&      meas,
    ...
    for (int iter = 0; iter < max_iter_; ++iter) {
      ...
      if (dp.norm() < tol_)
      {
        ROS_WARN("[TdoaWlsSolver] Converged in %d iterations (residual norm=%.3f), dp norm=%.3f", iter + 1, r.norm(), dp.norm());
        return true;
      }
    }
    return false;
...
// =============================================================================
// FgoState — state container for one FGO branch
// =============================================================================
struct FgoState {
  gtsam::Pose3                 pose;
...
class UwbImuFusionNode {
 public:
  ...
  ~UwbImuFusionNode() {
    if (traj_log_.is_open()) { traj_log_.flush(); traj_log_.close(); }
    if (imu_log_.is_open())  { imu_log_.flush();  imu_log_.close();  }
  }
 ...
 private:
  ...
  void loadParams() {
    ...
    if (anchors_.empty()) { ROS_ERROR("[uwb_imu_fusion] No anchors!"); ros::shutdown(); return; }
  ...
  CycleImuStats computeCycleImuStats(double from_t, double to_t) const {
    ...
    for (const auto& imu : imu_buf_) {
      ...
      if (imu.t >  to_t)   break;
      s.mean_ax += imu.acc.x(); s.mean_ay += imu.acc.y(); s.mean_az += imu.acc.z();
      s.mean_gx += imu.gyr.x(); s.mean_gy += imu.gyr.y(); s.mean_gz += imu.gyr.z();
    ...
    if (s.n > 0) {
      ...
      s.mean_ax *= inv; s.mean_ay *= inv; s.mean_az *= inv;
      s.mean_gx *= inv; s.mean_gy *= inv; s.mean_gz *= inv;
  ...
  void addTcVerticalPrior(gtsam::NonlinearFactorGraph& graph,
                          ...
                          double z_ref) {
    if (!tc_use_vertical_prior_) return;
    gtsam::Vector6 sig;
    sig << 1e6, 1e6, 1e6,
  ...
  void addTcWlsPositionPrior(gtsam::NonlinearFactorGraph& graph,
    ...
    if (!tc_use_wls_position_prior_) return;
  ...
  bool wlsPassesInnovationGate(const Eigen::Vector3d& wls_p) const {
    if (wls_innovation_gate_ <= 0.0) return true;  // gate disabled
    const auto& tp = tc_.pose.translation();
